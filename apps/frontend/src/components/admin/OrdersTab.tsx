...
export function OrdersTab() {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    try { setOrders(await listOrders(statusFilter || undefined)); }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <h2 className="font-display text-2xl font-bold text-accent-charcoal">Orders <span className="text-accent-charcoal/30 text-base font-normal">({orders.length})</span></h2>
      ...
      <div className="space-y-2">
        ...
        {orders.length === 0 && <p className="text-center text-accent-charcoal/40 py-12">No orders found.</p>}
      </div>
    </div>
  );
}
